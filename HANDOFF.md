...
> **2026-07-17 update (v2.0.0, Claude Fable 5 session):** major overhaul since this
> brief was written — esports removed; 12 sports with verified fixtures; sports data
> now generated by `scripts/build-sports-data.mjs` and delivered over the air via
> `website/data/sports/v2/catalog.json` (bundled snapshot as fallback); follow model
> + Up Next rail + session reminders + .ics export; sha256-verified updates; CI
> workflows; calmer in-app copy. See `docs/ROADMAP.md` (audit + plan) and
> `docs/DISTRIBUTION.md` (signing/winget/GitHub-Releases owner actions).
> Sections below describing `src/js/sports/catalog.js` internals are outdated.
